...
var Mdjs = (function () {
    ...
    Mdjs.prototype.render_markdown = function render_markdown() {
        ...
        renderer.heading = function (text, level) {
            ...
            // 使用text文本来写hash
            var hash = '<a name="' + text + '" id="' + text + '"></a>';
    ...
    Mdjs.prototype._init_express = function _init_express() {
        ...
        app.get(/(^\/$|\/$)/, function (req, res, next) {
            ...
            var flag = false;
            _this2.options.default_index.some(function (val) {
                if (_fs.existsSync(_path.resolve(_this2.options.root, filepath, val))) {
                    flag = val;
                    return true;
                }
            });
        ...
        dir_data.concat(file_data).forEach(function (file) {
            if (file.type === 'dir') {
                ...
                result.children.push({
                    text: _this3.get_markdown_title(file.filepath),
                    uri: file.filepath.replace(options.root, '').split(_path.sep).join('\/')
                });
            }
        });
    ...
    Mdjs.prototype._md = function _md(req, res, next) {
        ...
        var markdown_data = this.render_markdown(_fs.readFileSync(filepath).toString()).content;
    ...
    Mdjs.prototype.get_markdown_title = function get_markdown_title(filepath) {
        // 如果是md扩展
